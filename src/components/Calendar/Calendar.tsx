...
//     })
  ...
  const calendarWeeks = useMemo(() => { // useMemo é utilizado para evitar que a função seja executada toda vez que o componente for renderizado
    ...
    const previousMonthFillArray = Array.from({ // O dia da semana sempre me retorna o que falta para preencher a linha
      ...
      const lastDayInCurrentMonth = currentDate.set( // pegando o último dia do mês
        'date',
      ...
      const calendarDays = [ // Pegando todos os dias do mes e formatando eles
      ...
      const calendarWeeks = calendarDays.reduce<CalendarWeeks>( // Calculando as semanas do mês
        (weeks, _, i, original) => {

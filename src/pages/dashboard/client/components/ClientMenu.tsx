            ...
            <DropdownMenuTrigger asChild>
                <Button variant={'ghost'} className="flex h-11 w-full items-center justify-start gap-3 px-8 py-7 max-lg:justify-center max-lg:px-0">
                    ...
                    <div className="flex w-44 flex-col justify-start max-sm:visible sm:hidden lg:block">
                        {profile && <h2 className="overflow-hidden overflow-ellipsis whitespace-nowrap text-left text-sm">{profile.user.name}</h2>}
                        {profile && <p className="overflow-hidden overflow-ellipsis whitespace-nowrap text-left text-xs">{profile.user.email}</p>}
                    </div>
                </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
